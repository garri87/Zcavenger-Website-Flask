...
def upload_profile_img(self,user=None,new_profileimg=None):
    now = datetime.now()                
    ...
    if new_profileimg.filename != "":
        ...
        if file_extension in settings.UPLOAD_EXTENSIONS:
            if user:
                if user.profileimg != "":
                    os.remove(os.path.join(settings.UPLOADS_FOLDER, user.profileimg))
                    user.profileimg = new_profileimg_name   
                new_profileimg.save(os.path.join(settings.UPLOADS_FOLDER, new_profileimg_name))
